...
import { annulAdvanceRole, applyAdvanceRoleSelection, approveAdvanceRole, closeAdvanceRole, createAdvanceRole, createAdvanceRoleBulk, decideAdvanceLine, deleteAdvanceRole, downloadAdvanceReport, downloadAdvanceRoleCsv, downloadAdvanceTemplate, fetchAdvanceNoveltyTypes, fetchAdvanceRoles, reverseApprovalAdvanceRole, updateAdvanceRole } from '../../services/advancePayrollApi';
...
function Beneficios() {
  ...
  const advanceActionMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      const labels = { delete: 'eliminado', annul: 'anulado', approve: 'aprobado', reverseApproval: 'devuelto a borrador', approveAndApply: 'aprobado y aplicado', apply: 'aplicado', decide: 'resuelto', close: 'cerrado' };
        ...
        <div className="space-y-4">
          {advanceRoles.length === 0 ? <p className="rounded-md border border-slate-200 border-dashed bg-white px-4 py-6 text-center text-sm text-slate-500">Todavía no hay roles de anticipos generados.</p> : paginatedRoles.map((sourceRole) => {
            ...
            return <article className="rounded-md border border-slate-200 bg-white p-4" key={role.id}>
              ...
              {role.estado !== 'cerrado' && role.estado !== 'anulado' && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
