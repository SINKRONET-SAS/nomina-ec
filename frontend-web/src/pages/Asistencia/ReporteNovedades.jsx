    ...
    <div className="space-y-6">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-4 md:grid-cols-5">
          <label className="text-sm font-medium text-slate-700">Mes<select className="mt-1 min-h-10 w-full rounded-md border border-slate-300 px-3" onChange={(event) => { setMes(Number(event.target.value)); setPage(1); }} value={mes}>{MONTHS.map((month, index) => <option key={month} value={index + 1}>{month}</option>)}</select></label>
          <label className="text-sm font-medium text-slate-700">Año<input className="mt-1 min-h-10 w-full rounded-md border border-slate-300 px-3" min="2020" max="2100" onChange={(event) => { setAnio(Number(event.target.value)); setPage(1); }} type="number" value={anio} /></label>
          <label className="text-sm font-medium text-slate-700">Estado<select className="mt-1 min-h-10 w-full rounded-md border border-slate-300 px-3" onChange={(event) => { setEstado(event.target.value); setPage(1); }} value={estado}><option value="">Todos</option><option value="pendiente">Pendiente</option><option value="aprobado">Aprobado</option><option value="rechazado">Rechazado</option><option value="anulado">Anulado</option></select></label>
          <label className="text-sm font-medium text-slate-700">Origen<select className="mt-1 min-h-10 w-full rounded-md border border-slate-300 px-3" onChange={(event) => { setOrigen(event.target.value); setPage(1); }} value={origen}><option value="">Todos</option><option value="manual">Manual</option><option value="carga_masiva">Carga masiva</option><option value="asistencia">Dia laborable calculated</option></select></label>
          <label className="text-sm font-medium text-slate-700 md:col-span-2 xl:col-span-1">Buscar<input className="mt-1 min-h-10 w-full rounded-md border border-slate-300 px-3" onChange={(event) => { setBuscar(event.target.value); setPage(1); }} placeholder="Empleado o cédula" value={buscar} /></label>
        </div>
      </section>
